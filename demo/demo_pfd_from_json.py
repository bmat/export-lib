...
from export_util.writer import XLSXBytesOutputWriter, OutputTemplate, PDFBytesOutputWriter
...
if __name__ == '__main__':
    ex = Exporter(
        ...
        output=PDFBytesOutputWriter(offsets=[0, 40, 450, 550, 650, 750, 800, 3000, 3050, 3100, 3200, 3600, 3800,
                                             4200, 4400, 4600, 4700, 4800, 4900, 5000, 5300, 5500],pagesize=(6000, 900),
                                    header=[('Cue Nr.',	'Musiktitelstück (Title)',	'TC Start',	'TC Ende (Finish)',	'Länge in Sek. (Duration)',	'Duration (s)',	'Komponist/Texter/Bearbeiter (Composer, Written by)',	'Nr. (Track Number on CD)',	'Nutzungsart (Type of use - ILLM (Background) or VOR (Title))',	'Musik Typ',	'Verlag (Publisher)',	'CD-Titel'	,'Interpret (Performer)'	,'Label',	'Label code',	'Gema Nr.',	'CDCatalog Nr.',	'ISRC',	'ISWC',	'BmatId',	'Video Link')])
    )

    with open('demo_pdf.pdf', 'wb') as f:
        data = json.load(open(os.path.join(RESOURCES, 'input.json'), 'r'))
        filename, mime, pdf_data = ex.generate(data)
        f.write(pdf_data)
