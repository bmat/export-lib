...
if __name__ == '__main__':
    ex = Exporter(
        normalizer=Normalizer(tpl.Object(
            col=1,
            titles=True,
            translate={
                'Duration': 'Another duration name',
                'production': {
                    'id_title': 'Production ID'
                },
                'misc': {
                    'isrc': 'ISRC TRANSLATED!!!',
                    'work': {
                        'id': 'Work ID',
                        'type': 'Work ID Type'
                    }
            ...
            fields=[
                # Here we replacing field name "production.id" => "Production ID" as the `translate` option will be
                # automatically converted to DataGetter object, so we can simply keep the path to any nested dictionary
                # object.
                tpl.Field(1, 'production.id_title', '_id.$oid'),

                # Any other field name which is not provided in the `translate` dict will be kept as is.
                tpl.Field(2, 'Source', 'cuesheet_channel'),

                # Here we replacing field name "Duration" => "Another duration name"
                ...
                tpl.Object(
                    ...
                    fields=[
                        ...
                        # here we using the translate dict to get field label
                        tpl.Field(11, 'misc.isrc', 'music_work.sound_recording_id.isrc'),
                        tpl.Field(12, 'misc.work.id', 'music_work.work_id.mpn_id'),
                        tpl.Field(13, 'misc.work.type', 'music_work.work_id.iswc'),
                        # end...
    ...
    with open('demo_trans.xlsx', 'wb') as f:
